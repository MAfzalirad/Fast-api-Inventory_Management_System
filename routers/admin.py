...
async def read_all(db: db_dependency, user: user_dependency, role_check: Annotated[dict, Depends(require_role('admin'))]):
    return db.query(Items).all()

@router.get('/items/{item_id}', status_code=status.HTTP_200_OK)
async def get_item(db: db_dependency, user: user_dependency, role_check: Annotated[dict, Depends(require_role('admin'))], item_id: int = Path(gt=0)):
